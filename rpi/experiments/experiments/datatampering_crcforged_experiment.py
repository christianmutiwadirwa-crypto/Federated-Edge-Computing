"""
=============================================================================
 datatampering_crcforged_experiment.py
 Implementation for the DataTampering (CRC Forged) cybersecurity experiment.
=============================================================================
"""
...
def _almost_valid_payload() -> bytes:
    ...
    correct_crc = _crc16_ccitt(body)
    return body + struct.pack("<H", correct_crc)

class DataTamperingCRCForgedExperiment(BaseExperiment):
    """
    ...
    Framework integration:
        The ExperimentManager automatically assigns the 'DataTamperingCRCForgedExperiment'
    ...
    def run(self) -> None:
        ...
        self.logger.info(f"[{self.name}] DATA TAMPERING (CRC FORGED) ATTACK against {self.target_ip}:{self.target_port}")
        ...
        # Exclusively send packets that are structurally valid and have perfectly forged CRCs
        ...
        while time.time() - start_time < attack_duration:
            ...
            try:
                ...
                if (i + 1) % 25 == 0:
                    self.logger.info(
                        f"  [{self.name}] {i + 1}/{self.num_packets} forged packets sent"
                    )
    ...
    def cleanup(self) -> None:
        ...
        self.logger.info(f"[{self.name}] Data Tampering (CRC Forged) complete: {self.sent} packets sent, {self.errors} errors.")
